...
public class Pattern implements PatternInterface {
  ...
  @Override
  public String doPattern1(int[][][] rgbBuffer, int noOfSquaresAcross) throws IOException {
    ...
    return printPattern(pattern_buffer, noOfSquaresAcross);
  }
  ...
  public String savePattern(int[][][] rgbBuffer, int noOfSquaresAcross, List<Legend> legendList) {
    ...
    pattern.append(String.format("%d x %d\n\n", n, m));
    /*
     * for (int y = 0; y < image_width; y += square_length) { for (int x = 0; x <
     * image_height; x += square_length) { for (int yd = y; ((yd < y +
     * square_length) && (yd < image_width)); yd++) { for (int xd = x; ((xd < x +
     * square_length) && (xd < image_height)); xd++) {
     * 
     * int centre_x = (square_length + x + x) / 2; int centre_y = (square_length + y
     * + y) / 2;
     * 
     * if (xd == centre_x && yd == centre_y) {
     * 
     * int redColor = rgbBuffer_copy[xd][yd][0]; int greenColor =
     * rgbBuffer_copy[xd][yd][1]; int blueColor = rgbBuffer_copy[xd][yd][2]; char
     * symbol = getSymbol(redColor, greenColor, blueColor, legendList);
